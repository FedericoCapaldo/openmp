...
void filterDynamic(int start_point, int end_point) {
  /* 3x3 Sobel mask for X Dimension. */
  maskX[0][0] = -1; maskX[0][1] = 0; maskX[0][2] = 1;
  maskX[1][0] = -2; maskX[1][1] = 0; maskX[1][2] = 2;
  maskX[2][0] = -1; maskX[2][1] = 0; maskX[2][2] = 1;
...
void compute_sobel_dynamic()
{
  ...
  #pragma omp parallel shared(maxChunk, total)
  {
    while(maxChunk >= 0) {


      int start = total;
      int end = std::min((start + chunkSize), image_height);

      filterDynamic(start, end);
